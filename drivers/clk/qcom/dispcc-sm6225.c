...
#include <dt-bindings/clock/qcom,dispcc-sm6225.h>
...
static int disp_cc_sm6225_probe(struct platform_device *pdev)
{
	...
	ret = qcom_cc_really_probe(&pdev->dev, &disp_cc_sm6225_desc, regmap);
	if (ret) {
...
static int __init disp_cc_sm6225_init(void)
{
	return platform_driver_register(&disp_cc_sm6225_driver);
}
subsys_initcall(disp_cc_sm6225_init);

static void __exit disp_cc_sm6225_exit(void)
{
	platform_driver_unregister(&disp_cc_sm6225_driver);
}
module_exit(disp_cc_sm6225_exit);

MODULE_DESCRIPTION("QTI DISP_CC SM6225 Driver");
