...
static int sy6974_fw_probe(struct sy6974_device *sy)
{
	...
	pr_warn("charge-current=%d, ichg=%d\n", property, sy->init_data.ichg);
	...
	pr_warn("shutdown-current=%d, sdchg=%d\n", property, sy->init_data.sdchg);
...
static void sy6974_remove(struct i2c_client *client)
{
