...
#if IS_ENABLED(CONFIG_TOUCHSCREEN_TBN_AOC_CHANNEL_MODE) && __has_include("aoc_tbn_service_dev.h")
#define TBN_HAS_AOC_SERVICE 1
#include <uapi/linux/sched/types.h>
#include "aoc_tbn_service_dev.h"

/*
 * Allow building TBN without the AOC service implementation linked-in.
 * If the real provider is present, its strong symbols override these.
 */
ssize_t __weak aoc_tbn_service_write(void *cmd, size_t size)
{
	return -ENODEV;
}

ssize_t __weak aoc_tbn_service_read(void *cmd, size_t size)
{
	return -ENODEV;
}

bool __weak aoc_tbn_service_ready(void)
{
	return false;
}
#endif
...
#if defined(TBN_HAS_AOC_SERVICE)
