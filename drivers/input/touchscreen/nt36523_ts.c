...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	ts->reset_gpio = of_get_named_gpio_flags(np, "reset-gpio", 0, &ts->reset_flags);
...
static irqreturn_t nvt_ts_work_func(int irq, void *data)
{
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
			...
			//printk("\nx: %d, y: %d", input_x, input_y);
			//input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, input_w);
			//input_report_abs(ts->input_dev, ABS_MT_PRESSURE, input_p);
			touchscreen_report_pos(ts->input_dev, &ts->props_ts, input_x, input_y, true);
			//touchscreen_report_pressure(ts->input_dev, input_p, true);
	...
	if (ts->pen_support && ts->pen_input_dev_enable && !(ts->pen_is_charge)) {
		...
		if (pen_format_id != 0xFF) {
			if (pen_format_id == 0x01) {
				...
				//touchscreen_report_pos(ts->pen_input_dev, &ts->props_pen, pen_x, pen_y, false);
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	ts->input_dev->phys = devm_kasprintf(dev, GFP_KERNEL,
				     "%s/input0", dev_name(dev));
	ts->input_dev->dev.parent = dev;
	ts->input_dev->id.bustype = BUS_SPI;

	touchscreen_parse_properties(ts->input_dev, true, &ts->props_ts);
	if (!ts->props_ts.max_x || !ts->props_ts.max_y) {
	...
	if (ts->pen_support) {
		...
		input_abs_set_res(ts->pen_input_dev, ABS_X, 10);
		input_abs_set_res(ts->pen_input_dev, ABS_Y, 10);
		...
		ts->pen_input_dev->phys = devm_kasprintf(dev, GFP_KERNEL,
					     "%s/input1", dev_name(dev));
		ts->pen_input_dev->dev.parent = dev;
		ts->pen_input_dev->id.bustype = BUS_SPI;

		//touchscreen_parse_properties(ts->pen_input_dev, false, &ts->props_pen);
		//if (!ts->props_pen.max_x || !ts->props_pen.max_y) {
		//	dev_err(&ts->pen_input_dev->dev,
		//		"Touchscreen-size-x and/or touchscreen-size-y not set in dts\n");
		//	return -EINVAL;
		//}
