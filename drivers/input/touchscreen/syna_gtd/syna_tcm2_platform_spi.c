...
#define SYNA_DEFAULT_FW_IMAGE_NAME FW_IMAGE_NAME
#else
#define SYNA_DEFAULT_FW_IMAGE_NAME "synaptics.img"
#endif
...
#ifdef DEV_MANAGED_API
		gpio_flags = (dir == 0) ? GPIOF_IN :
			     (state ? GPIOF_OUT_INIT_HIGH : GPIOF_OUT_INIT_LOW);
		retval = devm_gpio_request_one(dev, gpio, gpio_flags, label);
#else /* Legacy API */
		...
		struct device_node *np, int setting_id)
{
	...
	retval = of_property_read_string_index(np,
			...
			setting_id, &name);
	if (retval < 0) {
		strncpy(hw_if->fw_name, SYNA_DEFAULT_FW_IMAGE_NAME,
...
#endif
#if IS_ENABLED(CONFIG_SPI_S3C64XX_GS)
	s3c64xx_sdd = spi_controller_get_devdata(spi->controller);
